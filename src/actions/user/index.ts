        ...
        if(found){
            if(found.integrations.length>0){
                ...
                if(days < 5){
                    ...
                    const refreshedToken = await refreshToken(found.integrations[0].token);

                    const today = new Date();

                    const expire_date = today.setDate(today.getDate() + 60);
                    const update_token = await updateIntegration(refreshedToken, new Date(expire_date), found.integrations[0].id);
