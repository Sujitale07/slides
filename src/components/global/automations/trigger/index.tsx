...
const Trigger = ({ id }: Props) => {
  ...
  if (data?.automation && data?.automation?.trigger.length > 0) {
    return (
      <div className="flex flex-col ga-y-6 items-center">
        <ActiveTrigger
          type={data.automation.trigger[0].type}
          keywords={data.automation.keywords}
        />

        {data?.automation?.trigger.length > 1 && (
          <>
            ...
            <ActiveTrigger
              type={data.automation.trigger[1].type}
              keywords={data.automation.keywords}
            />
          </>
        )}

        {!data.automation.listener && <ThenAction id={id} />}
      </div>
    )
  }
  return (
